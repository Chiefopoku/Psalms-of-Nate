...
export function ExpandableGallery({ images, className = "" }: ExpandableGalleryProps) {
  ...
  const step = (e: React.MouseEvent, delta: number) => {
    e.stopPropagation();
    setSelectedIndex((i) => (i === null ? null : (i + delta + images.length) % images.length));
  };

  const flex = (i: number) => (hoveredIndex === null ? 1 : hoveredIndex === i ? 2 : 0.5);

  return (
    <div className={className}>
      <div className="flex gap-2 h-80 sm:h-96 w-full">
        {images.map((image, index) => (
          <motion.div
            key={index}
            className="relative cursor-pointer overflow-hidden rounded-md"
            style={{ flex: 1 }}
            animate={{ flex: flex(index) }}
